...
class NonLinearResponse3rdOrder:
    ...
    def get_frequency_nonuniform(self):
        ...
        self.frequency_12 = w0_pol3_12 + np.linspace(-N_comb * self.delta_freq, N_comb * self.delta_freq, N_comb + 1)[:, np.newaxis] + np.linspace(-0.5 * self.delta_freq, 0.5 * self.delta_freq, N_res)
        self.frequency_21 = w0_pol3_21 + np.linspace(-N_comb * self.delta_freq, N_comb * self.delta_freq, N_comb + 1)[:, np.newaxis] + np.linspace(-0.5 * self.delta_freq, 0.5 * self.delta_freq, N_res)
        ...
        self.frequency = np.sort(np.hstack([self.frequency_12.flatten(), self.frequency_21.flatten()]))
